...
def fit_elbo(
    ...
    for epoch in tqdm(range(epochs), leave=True, desc='Epochs'):
        ...
        for inputs, targets in tqdm(train_loader, leave=False, desc='Train'):
        ...
        total_batches = 0
        total_loss = 0.0
        for inputs, targets in tqdm(test_loader, leave=False, desc='Test'):
            inputs = inputs.view((-1, 28 * 28))  # reshaping to vector
            loss, y_predictions = loss_function(
                model, inputs, targets, return_predictions=True
            )
...
def load_mnist_datasets(
    ...
    limit_test_samples_to: int = 10_000,
    ...
    # limiting for faster testing
    indices = np.random.permutation(len(test_dataset.data))[:limit_test_samples_to]
...
def show_learning_curves(metrics: dict[dict[str, List[float]]]) -> plt.Figure:
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    for name, met in metrics.items():
        ax.plot(met["step"], met["loss"], label=name)
...
def show_accuracy_curves(metrics: dict[dict[str, List[float]]]) -> plt.Figure:
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    for name, met in metrics.items():
        ax.plot(met["step"], met["acc"], label=name)
    ax.set_xlabel("Training step")
