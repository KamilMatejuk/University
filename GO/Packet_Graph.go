...
func GenerateGraph(numberOfVertices int, numberOfExtraEdges int) Graph {
...
func (g Graph) showPretty() {
	...
	if err == nil {
		...
		width, err = strconv.Atoi(width_s)
	}
...
func (g *Graph) summary() {
	...
	for _, p := range g.receivedPackets {
		fmt.Printf("Pakiet %d szedł drogą: %d", p, allPackets[p][0])
		for _, v := range allPackets[p][1:] {
			fmt.Printf(" > %d", v)
		}
		fmt.Printf("\n")
	}
...
var MAX_SLEEP_TIME = 1 	   // sekund
var NUMBER_OF_VERTICES = 8 		// n
var NUMBER_OF_EXTRA_EDGES = 5 	// d
var NUMBER_OF_PACKETS = 5 		// k

func main() {
	// stwórz graf
	graph := GenerateGraph(NUMBER_OF_VERTICES, NUMBER_OF_EXTRA_EDGES)
	...
	go graph.receiver(NUMBER_OF_PACKETS)
