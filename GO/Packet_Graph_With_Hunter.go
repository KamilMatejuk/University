	...
	if v.currentPacket != 0 {
		...
		if len(allPackets[v.currentPacket]) >= PACKET_LIFETIME {
			...
			*receivingChannel <- -1
			...
			fmt.Printf("[Wierzchołek %d] \tPakiet %d złapany w pułapkę\n", v.index, v.currentPacket)
			v.currentPacket = 0
			receivedPackets++
			*receivingChannel <- -1
			v.trap = false
...
func GenerateGraph(numberOfVertices int, numberOfExtraEdges int, numberOfExtraReversedEdges int) Graph {
	...
	for i := 0; i < numberOfVertices; i++ {
		v := Vertex{i, []int{}, 0, []*chan int{}, []*chan int{}, false}
...
func (g *Graph) hunter() {
	rand.Seed(time.Now().UnixNano())
	i := 0
	for i < NUMBER_OF_PACKETS/3 {
		i++
		time.Sleep(time.Duration(rand.Float64() * 3.0 * float64(time.Second)))
		// postaw pułapkę
		j := int(rand.Float64() * float64(NUMBER_OF_VERTICES))
		g.vertices[j].trap = true
	}
}
