...
podane są wartości nominałów w dowolnej kolejności. W drugim wierszu wejścia 
...
int main()
{
    ...
    for(int i=0; i<ilosc_nominalow; i++){
        for(int j=1; j<ilosc_nominalow-i; j++){
            if(nominaly[j-1]<nominaly[j]){
                swap(nominaly[j-1],nominaly[j]);
            }
        }
    }
    int kwota;
